...
@app.get("/notifications/fetch", response_model=List[schemas.NotificationsBase])
async def get_notifications(user_id: int, db: Session = Depends(get_session)):
  notifications = NotificationRepo.fetch_by_user_id(db, user_id)
  if notifications is None:
    raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No notifications")
  return notifications

@app.post("/notifications/create")
async def create_notification(user_id: int, message: str, db: Session = Depends(get_session)):
  
  notification: schemas.NotificationCreate = schemas.NotificationCreate(
      user_id = user_id,
      message = message,
      time_sent=datetime.datetime.now()
  )
  db_notification = await NotificationRepo.create(db = db, notification=notification)
...
async def create_er_request(booking: BookingsBase, db: Session = Depends(get_session)):
...
async def cancel_er_booking(booking_id: int, db: Session = Depends(get_session)):
